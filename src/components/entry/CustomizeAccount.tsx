...
import { useUpdateUser } from '../../lib/hooks/useUpdateUser.tsx';
  ...
  <div className='w-18 h-18 absolute bottom-0 left-1/2 translate-x-[-50%] translate-y-8'>
    ...
    />
        ...
        <div className='flex flex-col w-full h-full gap-4'>
            ...
            <p className='pl-1 text-twitterTextAlt font-bold'>Enter Display name</p>
            <input value={inputDisplayName} onChange={(e) => setInputDisplayName(e.target.value)} className='w-full border border-twitterBorder focus:outline-none focus:ring-0 rounded-xl px-2 h-16' placeholder='Enter display name'/>
            </div>

            <div className='flex w-full flex-col gap-2'>
            <p className={`pl-1 ${userNameError ? "text-red-500" : "text-twitterTextAlt"} font-bold`}>Enter Username</p>
            <input value={inputUsername} onChange={(e) => setInputUsername(e.target.value)} className='w-full border border-twitterBorder focus:outline-none focus:ring-0 rounded-xl focus:border-(--color-main) px-2 h-16' placeholder='Enter username'/>
            </div>

            <div className='flex w-full flex-col gap-2'>
            <p className='pl-1 text-twitterTextAlt font-bold'>Enter Bio</p>
            <input value={inputBio} onChange={(e) => setInputBio(e.target.value)} className='w-full border border-twitterBorder focus:outline-none focus:ring-0 px-2 rounded-xl h-16' placeholder='Enter bio'/>
            </div>
        </div>


        <div onClick={() => handleSave()} className={`hover:cursor-pointer hover:bg-(--color-main)/75 w-full bg-(--color-main) text-twitterText flex items-center gap-2 justify-center h-10 rounded-full`}>
            <p className="">Save</p>
        </div>
